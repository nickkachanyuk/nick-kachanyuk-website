...
One explanation is that since this is a principal component variable it captures more variance in the data than a regular variable. Let's briefly explore how this variable was created via PCA.
...
```{r}
tanzania %>% ggplot(., aes(dense_populated_wells)) + geom_density(aes(fill = status_group)) + facet_grid(status_group ~.) + theme_bw() + labs(x = "other densely populated wells principal component")
```
...
```{r}
tanzania %>% ggplot(., aes(sparsely_pop_wells)) + geom_density(aes(fill = status_group)) + facet_grid(status_group ~.) + theme_bw() + labs(x = "other sparsely populated wells principal component")
```
...
It is also no surprise that this variable placed 7/12 on variable of importance plot which suggests that this variable can possibly be removed from further analysis in the future.
...
The FNR wells seem that they are mostly clustered in the northern and western regions of Tanzania. Looking at the functional well group we can see the two tails on the negative x-axis further supposrting the idea that functional wells tend to be located in southern and eastern regions of Tanzania rather than in the north and/or west.

Overall this variable is 3/12 most important variables.
